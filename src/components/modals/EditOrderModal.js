...
function EditOrderModal(props) {
  const orderData = props.orderOriginData;
  ...
  useEffect(() => {
    if (orderData.length != 0) {
      setDocID(orderData.docID);
      setOrderNumber(orderData.orderNumber);
      setTotalPrice(orderData.totalPrice);
      setOrderName(orderData.name);
      setContactNumber(orderData.contactNumber);
      let date = orderData.orderDate;
      setCurDate(new Date(date.replaceAll("/", "-")));
      ...
      if (orders.length) {
        ...
        for (let i = 0; i < orders.length; i++) {
          temp.push({
            menuName: orders[i].menuCat,
            menuID: orders[i].id,
            foodID: orders[i].items[0].id,
            foodName: orders[i].items[0].name,
            comment: orders[i].items[0].itemComment,
            quentity: orders[i].items[0].quentity,
            weight: orders[i].items[0].weight,
          });
        }
        setDataSource([...temp]);
      }
  ...
  const sendOrder = (data) => {
    ...
    setDoc(doc(q, docID), data)
      .then(() => {
        histroy.replace({ pathname: "/" });
        toast.success("Order Data is updated!", {
          position: toast.POSITION.TOP_RIGHT,
        });
